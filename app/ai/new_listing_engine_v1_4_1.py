...
New Listing Engine V1.4.1

V1.4.1 routes all market-data acquisition through Market Data V2 and preserves
unadjusted Close semantics. V1.3 confirmation and decision logic is retained.
...
def _age_class(days,cfg):
    ...
    if days>=cfg.early_listing_days:return "NEW_LISTING_MATURE"
...
def _confirmation(r, data, cfg):
    """V1.3 confirmation engine, preserved verbatim in decision semantics."""
    p=_f(r["Close"]); e20=_f(r["ema20"]); e50=_f(r["ema50"]); rsi=_f(r["rsi"]); vr=_f(r["volume_ratio"]); rs=_f(r["rs_20d"])
    ...
    checks={
        ...
        "breakout_confirmation": bool(math.isfinite(p) and math.isfinite(prior_high) and p>=prior_high),
        "volume_confirmation": bool(math.isfinite(vr) and vr>=cfg.confirmation_volume_ratio),
        "momentum_confirmation": bool(math.isfinite(rsi) and rsi>=50),
        "relative_strength_confirmation": bool(math.isfinite(rs) and rs>=0),
        "trend_confirmation": bool(math.isfinite(p) and math.isfinite(e20) and math.isfinite(e50) and p>e20>e50),
    }
...
def analyze_dataframe(ticker,df,benchmark_df=None,cfg=None,listing_price=None,listing_days=None,listing_date=None):
    cfg=cfg or NewListingConfig(); data=_clean_ohlcv(df); days=len(data)
    if data.empty:return {"engine":"New Listing Engine","version":"1.4.1","ticker":ticker,"signal":"NO_DATA","decision":"WAIT","score":0.0,"confidence":0.0}
    ...
    first_trading_close=_f(data["Close"].iloc[0]); cls=_age_class(actual_days,cfg)
    if cls=="MATURE":return {"engine":"New Listing Engine","version":"1.4.1","ticker":ticker,"signal":"ROUTE_TO_CCE","decision":"USE_CCE","trading_days":days,"listing_age_days":actual_days,"classification":cls,"listing_date":actual_listing_date,"first_trading_close":round(first_trading_close,4) if math.isfinite(first_trading_close) else None,"score":0.0,"confidence":0.0,"data_quality":"GOOD"}
    if cls in {"INSUFFICIENT_DATA","EARLY_LISTING"}:return {"engine":"New Listing Engine","version":"1.4.1","ticker":ticker,"signal":"INSUFFICIENT_DATA","decision":"WAIT","trading_days":days,"listing_age_days":actual_days,"classification":cls,"listing_date":actual_listing_date,"first_trading_close":round(first_trading_close,4) if math.isfinite(first_trading_close) else None,"score":0.0,"confidence":0.0,"data_quality":"INSUFFICIENT" if cls=="INSUFFICIENT_DATA" else "LIMITED"}
    f=build_features(data,benchmark_df,cfg); r=f.iloc[-1]; score,breakdown=_score(r); conf=_confidence(r,actual_days,score); regime=_regime(r); confirmation=_confirmation(r,f,cfg); decision="WATCH"
    ...
    require_confirmation=cfg.require_confirmation_early if cls=="NEW_LISTING_EARLY" else cfg.require_confirmation_mature
    if decision=="BUY_ON_CONFIRMATION":
        if cls=="NEW_LISTING_EARLY" and require_confirmation:
            strict_early=(confirmation["confirmed"] and int(confirmation["score"])>=int(cfg.early_min_confirmation_score) and (not cfg.early_require_breakout or confirmation["checks"]["breakout_confirmation"]) and (not cfg.early_require_volume or confirmation["checks"]["volume_confirmation"]))
            confirmation["early_listing_confirmed"]=bool(strict_early)
            decision="BUY" if strict_early else "BUY_ON_CONFIRMATION"
...
def analyze(ticker,cfg=None,benchmark=None,listing_price=None):
    ...
    if cfg.listing_history_period and cfg.listing_history_period!=cfg.period:
        ...
        if not wider.empty:age_df=wider
    bdf=download_history(benchmark,cfg,cfg.period) if benchmark else None
    return analyze_dataframe(ticker,df,bdf if bdf is not None and not bdf.empty else None,cfg,listing_price,listing_days=len(age_df),listing_date=age_df.index[0].date().isoformat())
...
def self_test():
    ...
    assert "confirmation" in result and result["confirmation"]["max_score"] == 6
    ...
    routed=analyze_dataframe("MATURE",_synthetic_data(100),cfg=cfg,listing_days=330); assert routed["signal"]=="ROUTE_TO_CCE"
    assert routed["trading_days"] == 100 and routed["listing_age_days"] == 330
    ...
    # Early-listing confirmation regression.
    dates=pd.bdate_range("2026-01-01",periods=100); close=np.linspace(100,130,100); close[-1]=135
    cdf=pd.DataFrame({"Open":close*0.995,"High":close*1.01,"Low":close*0.99,"Close":close,"Volume":np.r_[np.full(99,1_000_000.0),1_500_000.0]},index=dates)
    cres=analyze_dataframe("CONFIRM",cdf,cfg=cfg,listing_days=100)
    assert "confirmation" in cres and cres["confirmation"]["max_score"]==6

    print("SELF-TEST: PASS")

if __name__=="__main__":
    parser=argparse.ArgumentParser(description="AI Trader New Listing Engine V1.4.1"); parser.add_argument("--ticker",default="SPCX"); parser.add_argument("--benchmark",default=None); parser.add_argument("--period",default="1y"); parser.add_argument("--listing-price",type=float,default=None); parser.add_argument("--self-test",action="store_true"); args=parser.parse_args()
