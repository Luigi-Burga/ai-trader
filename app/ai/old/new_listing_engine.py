...
"""AI Trader - New Listing Engine (NLE).

Standalone engine for securities with limited post-listing history.  It does
not force a new listing through Characteristic Curve Engine (CCE), and it
never uses a fixed buy price.  Entry/risk levels are derived from ATR and
moving-average structure.

CLI:
    python -m app.ai.new_listing_engine --ticker SPCX --benchmark QQQ --period 1y

The module is deliberately independent of main.py, signal_engine.py and
watchlist_scanner.py until its standalone behavior is validated.
"""
...
def _clean_ohlcv(df: pd.DataFrame) -> pd.DataFrame:
    ...
    if isinstance(x.columns, pd.MultiIndex):
        wanted = {"open":"Open", "high":"High", "low":"Low", "close":"Close", "volume":"Volume", "adj close":"Close"}
        selected = {}
...
def download_history(ticker: str, cfg: NewListingConfig) -> pd.DataFrame:
    if yf is None:
        return pd.DataFrame()
    try:
        return _clean_ohlcv(yf.download(ticker, period=cfg.period, interval=cfg.interval,
...
def build_features(data: pd.DataFrame, benchmark_df: Optional[pd.DataFrame]=None, cfg: Optional[NewListingConfig]=None) -> pd.DataFrame:
...
def _levels(r: pd.Series, cfg: NewListingConfig) -> Dict[str,Optional[float]]:
    ...
    if not all(math.isfinite(v) for v in (p,atr)) or atr<=0:
        return {k: None for k in ["current_price","zone_low","zone_high","preferred_entry","stop_loss","tp_1","tp_2","tp_3"]}
    low = p-cfg.pullback_atr*atr
    high = p+0.25*atr
    preferred = p-0.5*atr
    if math.isfinite(e20) and p > e20+cfg.max_entry_extension_atr*atr:
        high=e20+0.25*atr; preferred=e20
    stop=low-cfg.stop_atr*atr
    return {"current_price":round(p,4),"zone_low":round(max(0,low),4),"zone_high":round(max(0,high),4),
            "preferred_entry":round(max(0,preferred),4),"stop_loss":round(max(0,stop),4),
            "tp_1":round(p+1.5*atr,4),"tp_2":round(p+2.5*atr,4),"tp_3":round(p+4*atr,4)}


def analyze_dataframe(ticker: str, df: pd.DataFrame, benchmark_df: Optional[pd.DataFrame]=None,
                      cfg: Optional[NewListingConfig]=None, listing_price: Optional[float]=None) -> Dict[str,object]:
    cfg=cfg or NewListingConfig(); data=_clean_ohlcv(df); days=len(data)
    if data.empty: return {"engine":"New Listing Engine","version":"1.0","ticker":ticker,"signal":"NO_DATA","decision":"WAIT","score":0.0,"confidence":0.0}
    cls=_age_class(days,cfg)
    if cls=="MATURE":
        return {"engine":"New Listing Engine","version":"1.0","ticker":ticker,"signal":"ROUTE_TO_CCE","decision":"USE_CCE","trading_days":days,"score":0.0,"confidence":0.0}
    if cls=="INSUFFICIENT_DATA":
        return {"engine":"New Listing Engine","version":"1.0","ticker":ticker,"signal":"INSUFFICIENT_DATA","decision":"WAIT","trading_days":days,"score":0.0,"confidence":0.0}
    f=build_features(data,benchmark_df,cfg); r=f.iloc[-1]; score,breakdown=_score(r); conf=_confidence(r,days,score); regime=_regime(r)
    decision="WATCH"
    ...
    if all(math.isfinite(v) for v in (p,atr,e20)) and p>e20+cfg.max_entry_extension_atr*atr and decision=="BUY_ON_CONFIRMATION": decision="BUY_ON_PULLBACK"
    return {"engine":"New Listing Engine","version":"1.0","ticker":ticker,"as_of":str(data.index[-1].date()),"trading_days":days,
            "classification":cls,"signal":decision,"decision":decision,"score":round(score,2),"confidence":conf,"regime":regime,
            "listing_price":round(float(listing_price),4) if listing_price is not None else None,
            "metrics":{"current_price":round(_f(r["Close"]),4),"ema20":round(_f(r["ema20"]),4),"ema50":round(_f(r["ema50"]),4),
                       "rsi":round(_f(r["rsi"]),2),"atr":round(_f(r["atr"]),4),"atr_pct":round(_f(r["atr_pct"])*100,2),
                       "return_5d":round(_f(r["return_5d"])*100,2),"return_20d":round(_f(r["return_20d"])*100,2),
                       "return_since_listing":round(_f(r["return_since_listing"])*100,2),"drawdown_from_high":round(_f(r["drawdown_from_high"])*100,2),
                       "recovery_from_low":round(_f(r["recovery_from_low"])*100,2),"volume_ratio":round(_f(r["volume_ratio"]),2),
                       "relative_strength_20d":round(_f(r["rs_20d"])*100,2)},
            "entry":_levels(r,cfg),"score_breakdown":{k:round(v,2) for k,v in breakdown.items()},
            "evidence":{"method":"post-listing price/volume/volatility/relative-strength","historical_pattern_matches":0,
                        "note":"CCE historical matches are intentionally not used for a new listing."},"config":asdict(cfg)}


def analyze(ticker: str, cfg: Optional[NewListingConfig]=None, benchmark: Optional[str]=None, listing_price: Optional[float]=None) -> Dict[str,object]:
    cfg=cfg or NewListingConfig(benchmark=benchmark); benchmark=benchmark or cfg.benchmark
...
def _synthetic_data(n=140, seed=7):
    rng=np.random.default_rng(seed); dates=pd.bdate_range("2026-01-01",periods=n)
    ret=rng.normal(0.001,0.025,n); ret[-25:]+=0.0015
    close=100*np.exp(np.cumsum(ret)); high=close*(1+rng.uniform(0.005,0.03,n)); low=close*(1-rng.uniform(0.005,0.03,n)); open_=close*(1+rng.normal(0,0.008,n)); vol=rng.integers(800_000,2_000_000,n).astype(float)
    return pd.DataFrame({"Open":open_,"High":high,"Low":low,"Close":close,"Volume":vol},index=dates)


def self_test():
    cfg=NewListingConfig()
    # 140 trading days must route to NLE; feature calculations and dynamic levels must be finite.
    df=_synthetic_data()
    result=analyze_dataframe("TEST",df,cfg=cfg,listing_price=100)
    assert result["classification"]=="NEW_LISTING", result
    assert result["decision"] in {"WAIT","WATCH","AVOID","BUY_ON_CONFIRMATION","BUY_ON_PULLBACK"}, result
    ...
    blocked=analyze_dataframe("SHORT",short,cfg=cfg)
    assert blocked["signal"]=="INSUFFICIENT_DATA", blocked
    # 330 days must route to CCE.
    mature=_synthetic_data(330)
    routed=analyze_dataframe("MATURE",mature,cfg=cfg)
    ...
    lowvol_result=analyze_dataframe("LOWVOL",lowvol,cfg=cfg)
    assert lowvol_result["entry"]["zone_low"] != result["entry"]["zone_low"]
    print("SELF-TEST: PASS")
    print(json.dumps({k:result[k] for k in ("ticker","classification","trading_days","decision","score","confidence","regime","entry")},indent=2))


if __name__=="__main__":
    parser=argparse.ArgumentParser(description="AI Trader New Listing Engine")
    parser.add_argument("--ticker",default="SPCX"); parser.add_argument("--benchmark",default=None); parser.add_argument("--period",default="1y")
    parser.add_argument("--listing-price",type=float,default=None); parser.add_argument("--self-test",action="store_true")
    args=parser.parse_args()
    if args.self_test: self_test()
    else:
        result=analyze(args.ticker,NewListingConfig(period=args.period,benchmark=args.benchmark),args.benchmark,args.listing_price)
