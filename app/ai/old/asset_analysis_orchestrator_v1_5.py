...
from app.ai.asset_age_resolver_v1_4 import (
    ResolverConfig,
    resolve_asset_age,
...
def _engine_levels(result: Dict[str, Any]) -> Dict[str, Any]:
    ...
    NLE outputs are passed through using the same best-effort normalization.
    """
    ...
    return {
        ...
        "entry_price": first("entry_price", "current_price"),
        "entry_zone_low": first(
            "entry_zone_low", "historical_pullback_low"
        ),
        "entry_zone_high": first(
            "entry_zone_high", "historical_pullback_high"
        ),
        "stop": first(
            "stop", "dynamic_stop", "historical_stop_reference"
        ),
        "take_profit_1": first(
            "take_profit_1", "historical_tp_50", "tp1"
        ),
        "take_profit_2": first(
            "take_profit_2", "historical_tp_75", "tp2"
        ),
        "take_profit_3": first(
            "take_profit_3", "historical_tp_90", "tp3"
        ),
...
def main() -> None:
    parser = argparse.ArgumentParser(
        description="AI Trader Asset Analysis Orchestrator V1.5.0"
    )
