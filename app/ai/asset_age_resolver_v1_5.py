"""
Asset Age Resolver V1.5
=======================

Purpose
-------
Routes an asset to the appropriate historical-analysis engine based on
the ACTUAL feature matrix used by Characteristic Curve Engine V1.7.

V1.5 architectural correction
-----------------------------
V1.4 estimated ``valid_feature_rows`` with a simple Close warm-up formula:
...
V1.5 removes that duplicated approximation. It reuses the exact CCE V1.7
...
Important
---------
- This version does NOT modify CCE V1.7.
...
Compatibility
-------------
The public functions remain compatible with the V1.4-style resolver:
...
# The primary path imports the exact CCE V1.7 builder.
# ---------------------------------------------------------------------------
...
class ResolverConfig:
    """Configuration for Asset Age Resolver V1.5."""
...
def _flatten_yfinance_columns(df: pd.DataFrame) -> pd.DataFrame:
    ...
    try:
        # Prefer the exact CCE V1.7 cleaner.
        from app.ai.characteristic_curve_v1_7 import _clean_ohlcv
...
def _load_exact_cce_builder():
    """
    Return the exact CCE V1.7 cleaner and feature builder.

    V1.5 deliberately fails closed when exact reuse is required. This prevents
...
def _build_exact_features(
    ...
    """
    Construct the feature matrix using the exact CCE V1.7 implementation.

    The benchmark is optional. When absent, CCE V1.7 itself sets
    ...
    if clean_fn is None or build_fn is None or feature_names is None:
        if cfg.require_exact_cce_builder:
            raise RuntimeError(
                "CCE V1.7 exact feature builder is unavailable: "
                f"{import_error}"
            )
    ...
    if clean_asset.empty:
        return pd.DataFrame(), {
            "builder": "characteristic_curve_v1_7.build_features",
            "builder_version": "1.7",
            "exact_builder": True,
    ...
    if missing_features:
        raise RuntimeError(
            "CCE V1.7 feature matrix is missing expected features: "
...
def _base_result(
    ...
    *,
    ...
    return {
        ...
        # V1.5: this is now the ACTUAL CCE feature count.
...
def _build_resolution(
    ...
    """Build a V1.5 routing decision from an already downloaded DataFrame."""
    ...
    # ------------------------------------------------------------------
    # V1.5: early-history route remains unchanged.
    # ------------------------------------------------------------------
    ...
    # ------------------------------------------------------------------
    # V1.5 exact CCE feature construction.
    # ------------------------------------------------------------------
    ...
    try:
        ...
        return _base_result(
            ...
            reason=(
                "Exact CCE V1.7 feature construction failed. "
                f"{exc}"
            ),
            recommendation="REVIEW_DATA",
            cce_eligible=False,
            ...
            feature_builder={
                "builder": (
                    "characteristic_curve_v1_7.build_features"
                ),
                "builder_version": "1.7",
                "exact_builder": False,
                "error": str(exc),
    ...
    # ------------------------------------------------------------------
    # V1.5 structural CCE eligibility
    #
...
def resolve_asset_age(
    ...
    ``benchmark`` / ``benchmark_df`` are optional because CCE V1.7 can
...
def _self_test() -> int:
    ...
    print("Asset Age Resolver V1.5 self-test: PASS")
    print(
...
def main() -> int:
    parser = argparse.ArgumentParser(
        description="Asset Age Resolver V1.5"
    )
