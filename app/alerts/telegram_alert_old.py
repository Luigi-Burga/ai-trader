...
class _TelegramDispatcher:
    ...
    async def _get_updates_async(self, offset: Optional[int] = None) -> list:
    ...
    def get_updates(self, offset: Optional[int] = None) -> list:
        ...
        future: Future = asyncio.run_coroutine_threadsafe(
            self._get_updates_async(offset),
            loop,
        )
...
def telegram_wait_for_approval(
    ...
    """
    Wait for APPROVE <token> or REJECT <token> from the configured chat.

    Uses the SAME Bot instance/event loop as send_telegram().
    Returns:
        APPROVED
        REJECTED
        TIMEOUT
    ...
    while time.monotonic() < deadline:
        ...
        for update in updates:
            ...
            # Access the same configured chat used by the existing dispatcher.
            ...
            if text.upper() == f"APPROVE {token}".upper():
                return "APPROVED"
