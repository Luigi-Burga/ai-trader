...
def _build_market_data_cache(
    ...
    """
    Build one broad Market Data V2 request per unique ticker.

    V1.3 deliberately requests the same 5y/raw/1d shape already used by the
    watchlist pipeline. When that persistent cache exists, the Prediction
    Tracker avoids the prior narrow start/end Yahoo request entirely.

    Each ticker's broad frame is sliced to the exact prediction windows during
    evaluation, preserving V1.1 calculations. If a prediction falls outside
    the broad frame, _trading_days_since() performs the exact-window fallback.
    """
    ...
    print(
        f"[PERF-TRACKER] V1.3 cache strategy | "
        f"persistent_broad_request=5y | auto_adjust=False",
        flush=True,
    )
    ...
    for ticker in sorted(windows):
        ...
        try:
            # Critical V1.3 optimization:
            # reuse the persistent 5y Market Data V2 cache. This is the same
            # raw daily shape already requested by the watchlist scanner.
            frame = get_history(
                ticker,
                period="5y",
                start=None,
        ...
        print(
            f"[PERF-TRACKER] Market Data V1.3 | ticker={ticker} | "
            f"broad=5y | required_start={start} | required_end={end} | "
            f"rows={rows} | elapsed={elapsed:.2f}s",
            flush=True,
        )
    ...
    print(
        ...
        f"unique_tickers={len(windows)} | strategy=5y-cache-reuse",
        flush=True,
    )
...
def _self_test() -> int:
    ...
    # V1.3 cache-window slicing test.
