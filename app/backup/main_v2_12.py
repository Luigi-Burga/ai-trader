"""
AI Trader - Integrated Main V2.12
--------------------------------
...
- Order type is LIMIT using the scanner current_price.
- Default quantity is 1 share per eligible signal.
- Quantity can be overridden per run with:
      AI_TRADER_EXECUTION_QTY=<positive number>
...
def _execution_quantity() -> float:
    """
    Return the configured execution quantity.

    V2.12 defaults to one share as the controlled integration sizing.
    """
    raw = os.getenv(
        "AI_TRADER_EXECUTION_QTY",
        "1",
    ).strip()
    ...
    try:
        ...
        raise ValueError(
            "AI_TRADER_EXECUTION_QTY must be a positive number."
        ) from exc

    if quantity <= 0:
        raise ValueError(
            "AI_TRADER_EXECUTION_QTY must be greater than zero."
        )
...
def _build_trade_intent(
    ...
    """
    Convert an eligible scanner BUY result into a TradeIntent.

    The scanner remains responsible for the trading decision.
    V2.12 only maps its validated output into the execution contract.

    Rules:
    - BUY and STRONG_BUY are executable.
    - BUY_ON_CONFIRMATION is deliberately NOT executable.
    - current_price is required because V2.12 uses a LIMIT order.
    - confidence comes from the scanner.
    - quantity defaults to one share.
    """
    final_signal = str(
    ...
    if price is None:
        raise ValueError(
            f"{symbol}: executable signal {final_signal} has no current_price."
        )
    ...
    try:
        ...
        raise ValueError(
            f"{symbol}: invalid current_price={price!r}."
        ) from exc

    if limit_price <= 0:
        raise ValueError(
            f"{symbol}: current_price must be greater than zero."
        )

    confidence = result.get("confidence")
    if confidence is None:
        raise ValueError(
            f"{symbol}: executable signal has no confidence."
        )
...
def _execute_watchlist_signal(
    ...
    *,
    ...
    try:
        ...
        print(
            f"Estimated Notional: "
            f"${intent.notional_value:,.2f}"
        )
        print(
            f"Confidence: "
