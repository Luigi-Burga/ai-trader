...
def analyze_dataframe(
    ...
    cutoff = candidates[min(len(candidates)-1, max(cfg.min_matches-1, int(len(candidates)*cfg.match_quantile)))][2]
    eligible = [p for p in candidates if p[2] <= cutoff]
    dist_map = {p[0]: p[2] for p in eligible}
    selected_positions = _select_spaced([p[0] for p in eligible], dist_map, cfg.max_matches, cfg.min_gap_days)
    ...
    # Confidence rewards sample size, similarity and consistency.
    mean_distance = float(np.mean([r["distance"] for r in records]))
    similarity = math.exp(-mean_distance / 5.0)
