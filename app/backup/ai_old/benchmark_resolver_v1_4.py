...
Design goals
------------
1. Preserve explicit benchmark mappings.
2. Validate benchmark data quality and liquidity.
3. Adapt history/overlap requirements for young listings.
4. Select the most statistically relevant valid sector candidate.
5. Fall back to SPY when no sector benchmark is suitable.
6. Never silently replace an explicitly requested/manual benchmark.
7. Expose technical validity separately from statistical relevance.

CLI examples
-------------
python -m app.ai.benchmark_resolver_v1_4 --ticker SPCX --validate --listing-days 64
...
class ValidationConfig:
    ...
    # A relevance calculation is statistically useful with at least this
    # many paired return observations.
...
def _download_close_ohlcv(ticker: str, period: str = "5y") -> pd.DataFrame:
    """Download OHLCV while normalizing yfinance's possible MultiIndex output."""
    ...
    if isinstance(df.columns, pd.MultiIndex):
        # Prefer the requested ticker level when present.
        level_values = [
            list(map(str, df.columns.get_level_values(i)))
        ...
        if selected is not None:
            try:
                df = df.xs(selected, axis=1, level=0, drop_level=True)
            except Exception:
                try:
                    df = df.xs(selected, axis=1, level=1, drop_level=True)
                except Exception:
                    pass

    # If a MultiIndex remains, flatten it safely.
    if isinstance(df.columns, pd.MultiIndex):
        df.columns = [
            str(c[0]) if str(c[0]).lower() in {"open", "high", "low", "close", "volume"}
            else str(c[-1])
            for c in df.columns
        ]
...
def _requirements(
    ...
    """
    Determine adaptive benchmark requirements.

    For a mature asset, use the strict 252-session requirement.

    For a young listing, require approximately half the available asset
    history, bounded between 30 and 60 sessions. This prevents a new
    listing from failing merely because 252 benchmark sessions cannot
    overlap with a 30- or 60-session asset history.
    """
    ...
    history = max(config.young_min_history_days, overlap)

    return ValidationRequirements(
        required_history_days=history,
...
def _relevance_score(
    ...
    """
    Convert correlation evidence into a 0-100 relevance score.

    The score rewards consistency across daily, short-window and rolling
    correlation rather than relying on a single correlation statistic.
    """
    values = [daily_corr, short_corr, rolling_corr]
    if any(not np.isfinite(v) for v in values):
        return 0.0

    # Correlation is signed. Relevance is about co-movement, so positive
    # correlations are rewarded while negative values are not.
    positive = [max(0.0, min(1.0, v)) for v in values]

    # Weighted toward the full-history and rolling relationships.
    score = (
        positive[0] * 40.0
        + positive[1] * 30.0
        + positive[2] * 30.0
...
def _prepare_overlap(
    ...
    bench = benchmark_df[["Close"]].rename(columns={"Close": "benchmark_close"})

    joined = asset.join(bench, how="inner").dropna()
    return joined
...
def validate_benchmark(
    ...
    *,
    ...
    """
    Validate a benchmark against an asset.

    Returns:
        (BenchmarkValidation, overlap_dataframe)
    """
    ...
    if asset_df is None:
        asset_df = _download_close_ohlcv(ticker, period=period)

    if benchmark_df is None:
        benchmark_df = _download_close_ohlcv(benchmark, period=period)
    ...
    # Benchmark OHLCV quality.
    if "Volume" not in benchmark_df.columns:
        ohlcv_ok = False
        positive_volume_ratio = 0.0
        median_dollar_volume = 0.0
    else:
        volume = pd.to_numeric(
            benchmark_df["Volume"], errors="coerce"
        )
        close = pd.to_numeric(
            benchmark_df["Close"], errors="coerce"
        )

        valid_ohlcv = close.notna() & volume.notna() & (volume >= 0)
        quality_ratio = float(valid_ohlcv.mean()) if len(valid_ohlcv) else 0.0
    ...
    if "Volume" in benchmark_df.columns:
        ...
        quality_ratio = float(valid.mean()) if len(valid) else 0.0
    else:
        quality_ratio = 0.0
    ...
    if overlap_rows:
        asset_close = overlap["asset_close"]
        benchmark_close = overlap["benchmark_close"]

        close_valid = (
            asset_close.notna()
            & benchmark_close.notna()
            & np.isfinite(asset_close)
            & np.isfinite(benchmark_close)
        )

        overlap_close_ratio = (
            float(close_valid.mean()) if len(close_valid) else 0.0
        )
    else:
        overlap_close_ratio = 0.0
...
def calculate_relevance(
    ...
    *,
    ...
    """Calculate statistical benchmark relevance from overlapping closes."""
    config = config or ValidationConfig()
    ...
    rolling_corr = (
        float(rolling_corr_series.tail(1).iloc[0])
    ...
    reason = (
        "relevance_passed"
        if valid
        else "low_statistical_relevance"
    )
    ...
    return BenchmarkRelevance(
        ...
        beta=round(float(beta), 6) if np.isfinite(beta) else 0.0,
        observations=observations,
        role=role,
        quality_valid=quality_valid,
        valid=valid,
...
def resolve_benchmarks(
    ...
    *,
    ...
    Selection priority:
      ...
      2. Explicit benchmark map.
      3. Candidate sector benchmarks ranked by relevance.
      4. SPY broad-market fallback.

    A mapped benchmark is treated as the requested sector benchmark. If it
    fails technical validation or statistical relevance, candidate/fallback
    logic can be used unless the user explicitly supplied --benchmark.
    """
    config = config or ValidationConfig()
    ticker = _clean_ticker(ticker)
    ...
    if asset_df.empty:
        # Build a deterministic no-data response.
        req = _requirements(0, listing_days, config)
        empty_validation = BenchmarkValidation(
    ...
    if benchmark:
        ...
        role = _role_for_benchmark(ticker, manual)
        ...
        # Manual benchmark is never silently replaced.
        confidence = relevance.relevance_score if relevance.observations else 0.0

        if validation.valid:
            if relevance.valid:
                reason = "manual_benchmark_valid_and_relevant"
            else:
                reason = "manual_benchmark_valid_low_relevance"
        else:
            reason = "manual_benchmark_failed_validation"
        ...
        return _result_dict(
            ...
            role=role,
            is_manual=True,
            fallback_reason="" if validation.valid else validation.reason,
    ...
    # A mapped benchmark is evaluated first. For candidate-based mappings,
    # keep the mapped benchmark in the candidate set so that statistical
    # relevance determines the final sector benchmark.
    ...
    if market_validation.valid:
        if market_relevance.valid:
            selection_reason = "broad_market_fallback"
            confidence = market_relevance.relevance_score
        else:
            selection_reason = "broad_market_fallback"
            # The benchmark is technically valid but statistically weak.
            # Keep its relevance score visible rather than pretending it is
            # a strong sector relationship.
            confidence = market_relevance.relevance_score
        fallback_reason = ""
    else:
    ...
    return _result_dict(
        ...
        selected_benchmark=market if market_validation.available else "",
...
def validate_resolution(
    ...
    *,
    ...
    """Compatibility helper returning the complete resolver result."""
