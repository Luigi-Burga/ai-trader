...
Design:
    ticker
      ...
      +--> usable trading history
              ...
              +--> >= 12 months    -> CHARACTERISTIC_CURVE_ENGINE
...
- It can recommend CCE for intermediate-age assets only when enough feature
...
def _build_resolution(
    ...
    cce_eligible = (
        ...
        and trading_days >= cfg.mature_days
        and trading_days >= cfg.cce_min_feature_rows
    )
    ...
    if quality == "INVALID":
        ...
        if cce_eligible:
            ...
            reason = (
                f"{trading_days} valid trading days; history is intermediate "
                "and already satisfies the configured CCE depth."
            )
            ...
            reason = (
                f"{trading_days} valid trading days (6-12 month range); "
                "CCE minimum history is not yet satisfied."
            )
            recommendation = "USE_NLE"
        ...
        if cce_eligible:
            ...
            route = "MATURE"
            reason = (
                f"{trading_days} valid trading days (>= {cfg.mature_days}) "
                "and sufficient CCE history."
            )
            recommendation = "USE_CCE"
        else:
            engine = "NONE"
            route = "INSUFFICIENT_CCE_HISTORY"
            reason = (
                f"{trading_days} valid trading days but the CCE minimum "
                f"of {cfg.cce_min_feature_rows} usable rows is not satisfied."
            )
...
def main() -> int:
    parser = argparse.ArgumentParser(
        description="Asset Age Resolver V1.0"
    )
    parser.add_argument(
        "--ticker",
        required=True,
