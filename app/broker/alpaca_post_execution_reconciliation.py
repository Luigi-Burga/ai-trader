...
Semantic clarification in V1.1
------------------------------
OPEN + timeout is NOT a reconciliation failure.
It means the order remains open and no fill was confirmed within the
observation window.
...
OPEN_STATUSES = {
    "PENDING_NEW", "ACCEPTED", "NEW", "CALCULATED",
    "PENDING_REPLACE", "PENDING_CANCEL", "HELD", "SUSPENDED",
}
PARTIAL_STATUSES = {"PARTIALLY_FILLED"}
FILLED_STATUSES = {"FILLED"}
CANCELED_STATUSES = {"CANCELED", "CANCELLED"}
EXPIRED_STATUSES = {"EXPIRED", "DONE_FOR_DAY"}
...
class PostExecutionResult:
    ...
    def is_terminal_non_fill(self) -> bool:
        return self.execution_state in {"CANCELED", "EXPIRED", "REJECTED"}

    def summary(self) -> str:
        lines = [
            "ALPACA POST-EXECUTION RECONCILIATION V1.1",
            "=" * 60,
            "",
...
class AlpacaPostExecutionReconciliation:
    """
    Post-submission reconciliation against the broker state.
    ...
    def _build_result(
        ...
        *,
        ...
        filled_qty = self._to_float(self._value(order, "filled_qty", 0))
        requested_qty = self._to_float_or_none(self._value(order, "qty", None))
        ...
        if state == "FILLED":
            message = (
                "Order is FILLED at Alpaca. "
                "Broker execution is confirmed."
            )
            ...
            if timed_out:
                message = (
                    "Order remains OPEN at Alpaca. No fill was confirmed "
                    "within the post-execution observation window. "
                    "This is NOT a reconciliation failure and the order "
                    "has NOT been treated as filled."
                )
            else:
                message = (
                    "Order remains OPEN at Alpaca. No fill has been "
                    "confirmed yet. The order has NOT been treated as filled."
                )
        elif state == "CANCELED":
            message = (
                "Order reached terminal state CANCELED at Alpaca. "
                "No final fill was confirmed."
            )
        elif state == "EXPIRED":
            message = (
                "Order reached terminal state EXPIRED/DONE_FOR_DAY at Alpaca. "
                "No final fill was confirmed."
            )
        elif state == "REJECTED":
            message = (
                "Order reached terminal state REJECTED at Alpaca. "
                "No fill was confirmed."
            )
        else:
        ...
        return PostExecutionResult(
            ...
            timed_out=timed_out,
            reconciled=False,
            reconciliation_message="Not applicable for OPEN state.",
    ...
    def _finalize(self, result: PostExecutionResult) -> PostExecutionResult:
        ...
        if result.is_open:
            reconciliation_message = (
                "No dynamic portfolio reconciliation is performed while "
                "the order remains OPEN. Broker execution is still pending."
            )
        elif result.is_filled or result.is_partial:
            reconciliation_message = (
                "Dynamic broker-state reconciliation pending."
            )
        else:
            reconciliation_message = (
                "No dynamic portfolio reconciliation required for this "
