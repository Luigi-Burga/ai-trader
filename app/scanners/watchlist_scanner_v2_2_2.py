"""AI Trader - Watchlist Scanner V2.2.2

Production pipeline:
    watchlist -> Orchestrator V1.5.1 -> Decision Gate V1.5 -> Signal Engine V1.2

V2.2.2 integration fixes:
- Decision Gate V1.5 receives the authoritative Orchestrator signal.
- Stale engine_result signal fields cannot override the Orchestrator signal.
- Dynamic entry zones are normalized so low <= high before reaching the gate.
- NLE preferred entry / stop / target aliases are preserved through the gate.
- buy_target remains informational only and is never a BUY trigger.
"""
...
from app.strategies.decision_gate_v1_5 import apply_decision_gate
from app.strategies.signal_engine_v1_2 import generate_signal
...
def _gate_input(
    ...
    # V2.2 integration fix:
    # The Orchestrator exposes normalized decision-grade fields at the
    # analysis boundary, while engine_result remains the raw CCE/NLE payload.
    # Decision Gate V1.4 must receive the normalized APPT + dynamic levels.
    normalized_levels = analysis.get("levels")
    if isinstance(normalized_levels, Mapping):
        result["levels"] = dict(normalized_levels)

    if analysis.get("current_price") is not None:
        result["current_price"] = analysis.get("current_price")

    if analysis.get("appt_x_20d") is not None:
        result["appt_x_20d"] = analysis.get("appt_x_20d")

    if analysis.get("expected_return_20d") is not None:
        result["expected_return_20d"] = analysis.get("expected_return_20d")
...
def _print_result(
    ...
    appt = metrics.get("appt_x")
    dyn_ok = (gate.get("gates") or {}).get("dynamic_entry_stop")
    print(
        f"APPT_X_20D={_safe_float(appt):.4f}" if appt is not None
        else "APPT_X_20D=N/A"
    )
    print(
        f"dynamic_entry_stop_gate={dyn_ok}"
    )
    ...
    levels = analysis.get("levels") or engine.get("levels")
...
def scan_buy_opportunity(stock: Mapping[str, Any]) -> Dict[str, Any]:
    ...
    # V2.2.2 FIX:
    # The Orchestrator is authoritative for the selected engine's signal.
    # engine_result may contain legacy/raw fields whose signal semantics differ
    # from the normalized analysis boundary. Decision Gate V1.5 must never
    # re-interpret a stale signal from the nested engine payload.
    authoritative_signal = str(
        analysis.get("signal")
        or engine_result.get("operational_signal")
        or engine_result.get("signal")
        or "REVIEW_DATA"
    ).strip().upper()

    engine_result["engine_signal"] = authoritative_signal
    engine_result["signal"] = authoritative_signal
    engine_result["operational_signal"] = authoritative_signal

    # Normalize the dynamic level contract once more at the scanner boundary.
    # This protects Decision Gate V1.5 from malformed/inverted NLE zones.
    normalized_levels = engine_result.get("levels")
    if isinstance(normalized_levels, Mapping):
        ...
        high = levels.get("entry_zone_high")
        if low is not None and high is not None:
            low_f = _safe_float(low, float("nan"))
            high_f = _safe_float(high, float("nan"))
            if low_f == low_f and high_f == high_f and low_f > high_f:
                levels["entry_zone_low"], levels["entry_zone_high"] = (
                    high_f,
                    low_f,
                )

        # Decision Gate V1.5 consumes "stop"; the engine/orchestrator may expose
        # the same value as dynamic_stop or stop_loss.
        if levels.get("stop") is None:
            for key in ("dynamic_stop", "stop_loss"):
                if levels.get(key) is not None:
                    levels["stop"] = levels[key]
                    break

        # Preserve NLE/CCE aliases expected by downstream diagnostics.
        if levels.get("entry_price") is None:
            for key in ("preferred_entry", "current_price"):
                if levels.get(key) is not None:
                    levels["entry_price"] = levels[key]
                    break

        engine_result["levels"] = levels
    ...
    try:
        signal_result = generate_signal(
            ...
            account_equity=(
                _safe_float(stock.get("account_equity"))
                if stock.get("account_equity") is not None
                else None
            ),
            max_risk_pct=_safe_float(stock.get("max_risk_pct"), 1.0),
        )
    except Exception as exc:
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(
        description="AI Trader Watchlist Scanner V2.2.2"
    )
