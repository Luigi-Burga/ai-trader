...
4. BUY_ON_CONFIRMATION, BUY_ON_PULLBACK, WATCH, SELL, AVOID and REVIEW_DATA are preserved.
...
class GateConfig:
    ...
    # NLE pullback policy.
    # BUY_ON_PULLBACK is a conditional entry state, not an immediate BUY.
    nle_pullback_require_confirmation: bool = True
...
def _benchmark_gate(m: Mapping[str, Any], config: GateConfig, engine: str = "CCE") -> bool:
    ...
    # NLE is allowed to use a CONTEXT_ONLY benchmark as context. This does
    # not make the benchmark decision-grade; NLE must still pass its own
    # confidence, confirmation, regime and entry/risk gates.
    if trust in CONTEXT_ONLY_TRUST:
        return engine.upper() == "NLE"
...
def _evaluate_cce(
    ...
    trust_ok = _benchmark_gate(m, config, engine="CCE")
    fallback_ok = (
...
def _evaluate_nle(
    ...
    benchmark_ok = _benchmark_gate(m, config, engine="NLE")
...
def _evaluate_nle_pullback(
    m: Mapping[str, Any],
    config: GateConfig,
) -> Dict[str, Any]:
    """Evaluate BUY_ON_PULLBACK without converting it to an immediate BUY.

    For NLE pullbacks, ``entry_price`` may be below ``entry_zone_low``.
    The pullback zone is the trigger area, while ``entry_price`` is the
    preferred execution reference. The stop must remain below entry and
    below the upper edge of the zone.
    """
    confidence = m["trading_confidence"]
    confidence_ok = (
        confidence is not None
        and confidence >= config.min_nle_confidence
    )
    benchmark_ok = _benchmark_gate(m, config, engine="NLE")
    confirmation_ok = (
        _confirmation_gate(m, config)
        if config.nle_pullback_require_confirmation
        else True
    )
    regime_ok = m["regime"] not in config.blocked_regimes
    ...
    geometry_ok = all(
        ...
        for value in (entry, low, high, stop)
    )
    if geometry_ok:
        geometry_ok = (
            low <= high
            and stop < entry
            and stop <= high
            # NLE BUY_ON_PULLBACK: the preferred entry may be below
            # the pullback zone. The zone is the trigger area; entry is
            # the preferred execution reference.
            and entry <= high
        )

    current = m["current_price"]
    if not geometry_ok or current is None or current <= 0:
        price_state = "INVALID_PRICE_OR_GEOMETRY"
        price_in_zone = False
    elif current > high:
        price_state = "ABOVE_ENTRY_ZONE"
        price_in_zone = False
    elif current < low:
        price_state = "BELOW_ENTRY_ZONE"
        price_in_zone = False
    else:
        price_state = "INSIDE_ENTRY_ZONE"
        price_in_zone = True
    ...
    hard_gates = {
        ...
        "nle_confidence": confidence_ok,
        "confirmation": confirmation_ok,
        "dynamic_entry_stop_geometry": geometry_ok,
        "regime": regime_ok,
    }
    failed = [key for key, value in hard_gates.items() if not value]

    if failed:
        decision_path = "PULLBACK_GATE_BLOCK"
    elif price_state == "INSIDE_ENTRY_ZONE":
        decision_path = "PULLBACK_ZONE_REACHED"
    elif price_state == "ABOVE_ENTRY_ZONE":
        decision_path = "WAITING_FOR_PULLBACK"
    elif price_state == "BELOW_ENTRY_ZONE":
        decision_path = "BELOW_ENTRY_ZONE"
    else:
        decision_path = "PULLBACK_INVALID_GEOMETRY"

    return {
        "engine_policy": "NLE_PULLBACK",
        "all_passed": not failed,
        "hard_gates_passed": not failed,
        "hard_gates": hard_gates,
        "quality_gates": {"nle_confidence": confidence_ok},
        "gates": dict(hard_gates),
        "failed_gates": failed,
        "hard_failed_gates": failed,
        "quality_failed_gates": (
            [] if confidence_ok else ["nle_confidence"]
        ),
        "quality": {
            "confidence_ok": confidence_ok,
            "quality_mode": "NLE_PULLBACK",
        },
...
def build_decision(
    ...
    elif engine_signal == "BUY_ON_PULLBACK":
        pullback_gate = _evaluate_nle_pullback(m, config)
        operational_signal = "BUY_ON_PULLBACK"

        if pullback_gate["failed_gates"]:
            gate_status = "BLOCKED"
            gate_reason = "nle_pullback_gate_failed"
        else:
            price_state = pullback_gate["pullback"]["price_state"]
            if price_state == "INSIDE_ENTRY_ZONE":
                gate_status = "PULLBACK_ZONE_REACHED"
                gate_reason = "price_inside_pullback_entry_zone"
            elif price_state == "ABOVE_ENTRY_ZONE":
                gate_status = "WAITING_FOR_PULLBACK"
                gate_reason = "price_above_pullback_entry_zone"
            elif price_state == "BELOW_ENTRY_ZONE":
                gate_status = "BELOW_ENTRY_ZONE"
                gate_reason = "price_below_pullback_entry_zone"
            else:
                gate_status = "BLOCKED"
                gate_reason = "invalid_pullback_price_or_geometry"
...
def run_self_test() -> bool:
    ...
    case["benchmark_trust"] = "CONTEXT_ONLY"
    case["benchmark_trust_decision_grade"] = False
    cases.append(("NLE_CONTEXT_ONLY_ALLOWED", case, "BUY"))
    ...
    case = _base_nle()
    case["signal"] = "BUY_ON_PULLBACK"
    cases.append(("NLE_PULLBACK_ABOVE_ZONE", case, "BUY_ON_PULLBACK"))

    case = _base_nle()
    case["signal"] = "BUY_ON_PULLBACK"
    case["levels"]["current_price"] = 149.0
    cases.append(("NLE_PULLBACK_INSIDE_ZONE", case, "BUY_ON_PULLBACK"))

    case = _base_nle()
    case["signal"] = "BUY_ON_PULLBACK"
    case["benchmark_trust"] = "CONTEXT_ONLY"
    case["benchmark_trust_decision_grade"] = False
    cases.append(("NLE_PULLBACK_CONTEXT_ALLOWED", case, "BUY_ON_PULLBACK"))
