...
class GateConfig:
    ...
    # APPT is expressed in units of initial risk X.
    # Zero means the historical setup must have non-negative expectancy.
    ...
    metrics = {
        ...
        "current_price": _float(
            _get(
                ...
                "current_price",
                "price",
                default=(result.get("levels") or {}).get("current_price")
                if isinstance(result.get("levels"), Mapping)
                else None,
            )
        ),
        "entry_price": _float(
            _get(
                result,
                "entry_price",
                default=(result.get("levels") or {}).get("entry_price")
                if isinstance(result.get("levels"), Mapping)
                else None,
            )
        ),
        "entry_zone_low": _float(
            _get(
                result,
                "entry_zone_low",
                default=(result.get("levels") or {}).get("entry_zone_low")
                if isinstance(result.get("levels"), Mapping)
                else None,
            )
        ),
        "entry_zone_high": _float(
            _get(
                result,
                "entry_zone_high",
                default=(result.get("levels") or {}).get("entry_zone_high")
                if isinstance(result.get("levels"), Mapping)
                else None,
            )
        ),
        "stop": _float(
            _get(
                ...
                "stop",
                "dynamic_stop",
                default=(result.get("levels") or {}).get("stop")
                if isinstance(result.get("levels"), Mapping)
                else None,
            )
        ),
...
def evaluate_buy_gates(
    ...
    appt_ok = (
        appt_x is not None
        and appt_x >= config.min_appt_x
    )

    # Dynamic trade levels are part of the decision evidence.
    # For a bullish BUY to be operational, the orchestrator must provide a
    # coherent entry/stop structure. The current price must not be above the
    # dynamic entry-zone ceiling and must remain above the stop.
    ...
    dynamic_levels_valid = (
        dynamic_levels_present
        and current_price > 0
        and entry_price > 0
        and stop > 0
        and entry_zone_low > 0
        and entry_zone_high > 0
        and entry_zone_low <= entry_zone_high
        and stop < entry_price
        and entry_zone_low <= entry_price <= entry_zone_high
        and stop <= entry_zone_high
        and current_price >= stop
        and current_price <= entry_zone_high
    )

    dynamic_entry_stop_ok = dynamic_levels_valid
    ...
    *,
    ...
    result = {
        ...
        "levels": ({
            ...
            "stop": 95.0,
        } if levels == "DEFAULT" else (levels or {})),
    }
...
def run_self_test() -> bool:
    ...
    print("=" * 92)
    print("AI TRADER - DECISION GATE V1.3 SELF-TEST")
    print("=" * 92)
    print()
    print(
    ...
    print("DECISION GATE V1.3 SELF-TEST: PASS")
    return True
