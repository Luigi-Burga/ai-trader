...
def generate_signal(df=None, ticker=None, soxl_result=None, analysis_result=None,
                    benchmark_trust=None, characteristic_result=None):
    """
    ...
    """
    # Current watchlist_scanner.py calls generate_signal(characteristic_result=result).
    # Keep this keyword as an explicit compatibility alias for analysis_result.
    if analysis_result is None and characteristic_result is not None:
        analysis_result = characteristic_result

    # Characteristic Curve / NLE result path. The scanner expects a dict here
    # and reads final["signal"], so return the complete auditable decision.
    if isinstance(analysis_result, dict):
        ...
        decision["signal"] = decision["operational_signal"]
        decision["version"] = VERSION
        decision["analysis_result"] = analysis_result
        return decision
