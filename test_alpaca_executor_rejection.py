...
First Paper Order explicit rejection test.

Purpose
-------
Validate the COMPLETE Executor execution path up to the final safety gate,
while guaranteeing that Alpaca submit_order() cannot be called.

Test scenario
-------------
1. Risk Gate approves the TradeIntent.
2. Telegram approval returns APPROVED.
3. FIRST_ORDER_CONFIRMATION returns REJECTED.
4. Executor must return FIRST_ORDER_REJECTED.
...
class FakeTelegramApproval:
    """Deterministic Telegram double for the rejection test."""
...
class FakeClient:
    """Alpaca client double. submit_order is intentionally unavailable."""
...
def build_test_intent() -> TradeIntent:
    return TradeIntent(
        ...
        limit_price=180.50,
        time_in_force="day",
        reason="FIRST PAPER ORDER rejection test",
        confidence=84.2,
...
def build_approved_risk_decision(intent: TradeIntent) -> Any:
    """
    Build the minimum RiskDecision-compatible object required by the
    production Executor.

    We deliberately use SimpleNamespace instead of constructing the
    project's RiskDecision dataclass directly. This keeps this isolated
    safety test independent of non-essential RiskDecision constructor
    fields while preserving every attribute consumed by AlpacaExecutor.
    """
    return SimpleNamespace(
        approved=True,
...
def main() -> None:
    print("FIRST PAPER ORDER REJECTION TEST")
    print("=" * 60)
    print()
    ...
    print("First Order Confirmation:", "REJECTED")
    print()
    ...
    expected_status = "FIRST_ORDER_REJECTED"
